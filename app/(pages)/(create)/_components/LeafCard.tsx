...
export function LeafCard({ type, onLeafClick, isActive }: Props) {
  ...
  return (
    <button
      onClick={handleClick}
      className={`flex items-center justify-center rounded-md border-2 border-dashed ${isActive ? "border-dark-text bg-light-cream" : "border-action"} bg-cover transition duration-150`}
    >
